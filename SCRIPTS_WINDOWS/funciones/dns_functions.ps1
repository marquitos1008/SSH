# ==============================================
# FuncionesDNS.ps1
# Bibliotecas de funciones DNS para Windows Server 2022
# ==============================================
...
function Estado-DNS {
    ...
    if ($feature.Installed) { Write-Host "Rol DNS: INSTALADO" } else { Write-Host "Rol DNS: NO instalado" }

    $service = Get-Service DNS -ErrorAction SilentlyContinue
    if ($service -and $service.Status -eq "Running") { Write-Host "Servicio DNS: ACTIVO" } else { Write-Host "Servicio DNS: INACTIVO" }
...
function Configurar-Reprobados {
    ...
    if (!(Validar-IP $IPServidor)) { Write-Host "IP inválida."; Pause; return }
...
function Administrar-Dominios {
    ...
    switch ($op.ToUpper()) {
        ...
        "A" {
            ...
            if (!(Validar-IP $IPAsignada)) { Write-Host "IP inválida."; Pause; return }
            if (Get-DnsServerZone -Name $Dominio -ErrorAction SilentlyContinue) { Write-Host "La zona ya existe."; Pause; return }

            Add-DnsServerPrimaryZone -Name $Dominio -ZoneFile "$Dominio.dns" -DynamicUpdate None
            Add-DnsServerResourceRecordA -ZoneName $Dominio -Name "@" -IPv4Address $IPAsignada
            Add-DnsServerResourceRecordA -ZoneName $Dominio -Name "www" -IPv4Address $IPAsignada
            Restart-Service DNS
        ...
        "B" {
            ...
            } else { Write-Host "El dominio no existe." }
            Pause
        }

        # -------- LISTAR --------
        "C" {
            Write-Host ""; Write-Host "Zonas configuradas:"
            Get-DnsServerZone | Format-Table ZoneName, ZoneType -AutoSize
            Pause
        }

        # -------- REPROBADOS --------
        "D" { Configurar-Reprobados }
...
function Monitorear-DNS {
    ...
    if ((Get-Service DNS).Status -ne "Running") { Start-Service DNS }
    Write-Host ""; Write-Host "Zonas configuradas:"
